# -*- coding: utf-8 -*-
"""
main.py - Server WebSocket UNIFICATO (Garage + Arena) per il deploy
su Render.
...
class GameServer:
    ...
    async def handle_message(self, websocket, session: Session, raw: str):
        ...
        if msg_type == "login":
            ...
            token = accounts.create_session(self.conn_db, account["id"])
            await websocket.send(json.dumps({"type": "login_ok", "account": account, "token": token}))
            return

        if msg_type == "login_token":
            account = accounts.get_account_by_token(self.conn_db, msg.get("token", ""))
            if account is None:
                await websocket.send(json.dumps(make_error("Sessione scaduta, effettua di nuovo il login.")))
                return
            session.account_id = account["id"]
            session.username = account["username"]
            await websocket.send(json.dumps({"type": "login_ok", "account": account, "token": msg.get("token")}))
            return

        if msg_type == "logout":
            accounts.delete_session(self.conn_db, msg.get("token", ""))
            await websocket.send(json.dumps({"type": "logout_ok"}))
            return
        ...
        if msg_type == "join_arena":
            ...
            loadout = {
                ...
                "arm_left": robot["arm_left"], "arm_right": robot["arm_right"],
                "leg_left": robot["leg_left"], "leg_right": robot["leg_right"],

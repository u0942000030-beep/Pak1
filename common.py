...
FLOOR_Z = 0.0                    # celle: quota del pavimento. BUGFIX: prima non esisteva alcun limite inferiore per i proiettili con vera traiettoria verticale (laser, palla di fuoco della torre): mirando abbastanza in basso (pitch negativo), il colpo scendeva sotto zero e continuava a "cadere" all'infinito, passando attraverso il pavimento invece di fermarsi/esplodere li'. Per la palla di fuoco questo significava anche NESSUN danno ad area quando si mirava al pavimento sotto un nemico, perche' explode_fireball non veniva mai chiamata.
